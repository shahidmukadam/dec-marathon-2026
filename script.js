...
const dailyPlan = {
  ...
  3: createGymDay(
    "Lower Body",
    "Squat, hinge, calves, and ab work.",
    workoutTemplates.lowerBody,
    "2200",
    360,
  ),
  4: createRunDay({
  ...
  10: createGymDay(
    "Lower Body",
    "Repeat the lower-body strength session.",
    workoutTemplates.lowerBody,
    "2200",
    360,
  ),
  ...
  17: createGymDay(
    "Lower Body",
    "Lower-body strength with posterior-chain and calf work.",
    workoutTemplates.lowerBody,
    "2200",
    360,
  ),
  ...
  24: createGymDay(
    "Lower Body",
    "Repeat the lower-body strength session with solid form.",
    workoutTemplates.lowerBody,
    "2200",
    360,
  ),
...
function createDayCard(day, expanded) {
  ...
  card.className = `day-card is-${plan.category}${isMonday ? " is-monday" : ""}${entry.completed ? " is-complete" : ""}${day === calendarState.selectedDay ? " is-selected" : ""}${expanded ? " is-expanded" : ""}`;
...
function getCalendarViewCopy() {
  ...
  return {
    eyebrow: "Month View",
    note:
      "Month view shows the full June training grid. Click any card to set the current focus for week and day view, and log consumed calories to populate the report tab.",
  };
}
...
function renderCalendar() {
  ...
  visibleSlots.forEach((day) => {
    ...
    calendarGrid.appendChild(createDayCard(day, calendarState.view === "day"));
  });
}
...
calendarGrid.addEventListener("change", (event) => {
  ...
  target.closest(".day-card")?.classList.toggle("is-complete", target.checked);
});
